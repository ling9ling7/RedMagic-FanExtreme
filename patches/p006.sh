...
mkdir -p $MOD/.backup/p006
cp $MOD/service.sh $MOD/.backup/p006/service.sh 2>/dev/null
cat > $MOD/service.sh <<'FE_SVC_EOF'
#!/system/bin/sh
...
FE_SVC_EOF
grep -q 'MYST=' $MOD/service.sh || exit 1
